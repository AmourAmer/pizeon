...
// };
// use pizeon_common::{
//     api::{PIZEON_CARGO_VERSION, PIZEON_HEADER_VERSION, PIZEON_VERSION},
...
// 
// static APP_USER_AGENT: &str = concat!("pizeon/", env!("CARGO_PKG_VERSION"),);
// 
// pub struct Client<'a> {
//     sync_addr: &'a str,
...
//         .header(PIZEON_HEADER_VERSION, PIZEON_CARGO_VERSION)
//         .json(&map)
//         .send()
//         .await?;
// 
...
//     use pizeon_common::api::IndexResponse;
// 
//     let url = "https://api.pizeon.sh";
//     let client = reqwest::Client::new();
// 
//     let resp = client
//         .get(url)
...
//     let version = response.headers().get(PIZEON_HEADER_VERSION);
// 
...
//     if version.major < PIZEON_VERSION.major {
//         println!("Pizeon version mismatch! In order to successfully sync, the server needs to run a newer version of Pizeon");
//         println!("Client: {}", PIZEON_CARGO_VERSION);
...
//         bail!(
//             "Service unavailable: check https://status.pizeon.sh (or get in touch with your host)"
//         );
//     }
// 
...
//             }
// 
//             bail!("There was an error with the pizeon sync service, server error {status}: {reason}.\nIf the problem persists, contact the host")
//         }
// 
//         bail!("There was an error with the pizeon sync service: Status {status:?}.\nIf the problem persists, contact the host")
//     }
// 
//     Ok(resp)
// }
...
//         headers.insert(PIZEON_HEADER_VERSION, PIZEON_CARGO_VERSION.parse()?);
// 
//         Ok(Client {
//             sync_addr,
