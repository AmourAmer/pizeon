...
pub async fn list<DB: Database>(
    ...
    if req.sync_ts.unix_timestamp_nanos() < 0 || req.history_ts.unix_timestamp_nanos() < 0 {
        ...
        counter!("pizeon_history_epoch_before_zero", 1);
    ...
    counter!("pizeon_history_returned", history.len() as u64);
...
pub async fn add<DB: Database>(
    ...
    counter!("pizeon_history_uploaded", req.len() as u64);
    ...
    history.retain(|h| {
        ...
        if !keep {
            counter!("pizeon_history_too_long", 1);
