...
use pizeon_dotfiles::{shell::Alias, store::AliasStore};
...
impl Cmd {
    ...
    async fn import(&self, store: AliasStore) -> Result<()> {
        let aliases = pizeon_dotfiles::shell::import_aliases(store).await?;
    ...
    pub async fn run(&self, settings: &Settings, store: SqliteStore) -> Result<()> {
        if !settings.dotfiles.enabled {
            ...
            eprintln!("The default configuration file is located at ~/.config/pizeon/config.toml.");
            return Ok(());
        }
