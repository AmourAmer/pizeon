...
// use pizeon_common::shell::{shell_name, Shell};
...
// 
//     // The preexec framework used in the current session, if Pizeon is loaded.
...
//             .args([
//                 "-ic",
//                 format!("[ -z ${var} ] || echo PIZEON_DOCTOR_ENV_FOUND").as_str(),
//             ])
//             .output()
//             .map_or(String::new(), |v| {
//                 let out = v.stdout;
...
//             });
// 
//         cmd.contains("PIZEON_DOCTOR_ENV_FOUND")
//     }
// 
//     fn detect_preexec_framework(shell: &str) -> Option<String> {
//         if env::var("PIZEON_SESSION").ok().is_none() {
//             None
//         } else if shell.starts_with("bash") || shell == "sh" {
//             env::var("PIZEON_PREEXEC_BACKEND")
//                 .ok()
//                 .filter(|value| !value.is_empty())
//                 .and_then(|pizeon_preexec_backend| {
//                     pizeon_preexec_backend.rfind(':').and_then(|pos_colon| {
//                         u32::from_str(&pizeon_preexec_backend[..pos_colon])
//                             .ok()
...
//                             })
//                             .then(|| pizeon_preexec_backend[pos_colon + 1..].to_string())
//                     })
//                 })
//         } else {
...
//                 "pizeon",
//                 PluginShellType::Any,
//                 PluginProbeType::EnvironmentVariable("PIZEON_SESSION"),
//                 None,
//             ),
//             (
//                 "blesh",
...
// 
//     pub fn new() -> Self {
//         // TODO: rework to use pizeon_common::Shell
// 
...
// 
//         let parent = sys
//             .process(process.parent().expect("Pizeon running with no parent!"))
...
// #[derive(Debug, Serialize, Deserialize)]
// struct SyncInfo {
//     /// Whether the main Pizeon sync server is in use
//     /// I'm just calling it Pizeon Cloud for lack of a better name atm
//     pub cloud: bool,
//     pub records: bool,
//     pub auto_sync: bool,
// 
...
//         Self {
//             cloud: settings.sync_address == "https://api.pizeon.sh",
...
//     pub fn new(settings: &Settings) -> Self {
//         Self {
//             pizeon: PizeonInfo::new(settings),
//             shell: ShellInfo::new(),
...
//     println!(); // spacing
//                 //
//     let zfs_error = "[Filesystem] ZFS is known to have some issues with SQLite. Pizeon uses SQLite heavily. If you are having poor performance, there are some workarounds here: https://github.com/pizeonsh/pizeon/issues/952".bold().red();
//     let bash_plugin_error = "[Shell] If you are using Bash, Pizeon requires that either bash-preexec or ble.sh be installed. An older ble.sh may not be detected. so ignore this if you have it set up! Read more here: https://docs.pizeon.sh/guide/installation/#bash".bold().red();
//     let blesh_loading_order_error = "[Shell] Pizeon seems to be loaded before ble.sh is sourced. In .bashrc, make sure to initialize Pizeon after sourcing ble.sh.".bold().red();
// 
//     // ZFS: https://github.com/pizeonsh/pizeon/issues/952
...
//         }
// 
//         if info.shell.plugins.iter().any(|plugin| plugin == "pizeon")
