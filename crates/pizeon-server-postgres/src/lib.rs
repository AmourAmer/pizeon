...
use pizeon_common::record::{EncryptedData, HostId, Record, RecordIdx, RecordStatus};
use pizeon_server_database::models::{History, NewHistory, NewSession, NewUser, Session, User};
use pizeon_server_database::{Database, DbError, DbResult};
...
impl Database for Postgres {
    ...
    async fn add_records(&self, user: &User, records: &[Record<EncryptedData>]) -> DbResult<()> {
        ...
        for i in records {
            let id = pizeon_common::utils::uuid_v7();
