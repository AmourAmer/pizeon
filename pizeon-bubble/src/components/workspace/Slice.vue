...
<template>
  ...
  <textarea v-if="isTextarea()" :id="slice" v-model="model" :placeholder="initMsg()" />
  <select v-else-if="selectFor(slice).length > 0" :id="slice" v-model="model" multiple>
    <!-- TODO: shortcut multi-select -->
    <!-- TODO: default choice -->
    <option disabled value="">Please select one or more, I don't know</option>
    <option v-for="(option, i) in selectFor(slice)" :key="i">{{ option }}</option>
  </select>
